...
local function IsPlaceholderItemName(name, itemID)
    if not name or name == "" then return true end
    itemID = tonumber(itemID)
    if itemID and name == ("Item " .. itemID) then return true end
    if itemID and name == ("Item #" .. itemID) then return true end
    return false
end
...
local function ApplyLoadedItemData(itemID, itemData, loadedData, nameText, iconTexture)
    itemID = tonumber(itemID)
    if not itemID or not itemData or not loadedData then
        return false
    end

    local changed = false

    if loadedData.name and loadedData.name ~= "" and IsPlaceholderItemName(itemData.name, itemID) then
        itemData.name = loadedData.name
        changed = true
    end
    if loadedData.link and itemData.link ~= loadedData.link then
        itemData.link = loadedData.link
        changed = true
    end
    if loadedData.quality and itemData.rarity ~= loadedData.quality then
        itemData.rarity = loadedData.quality
        itemData.quality = loadedData.quality
        changed = true
    end
    if loadedData.icon and itemData.icon ~= loadedData.icon then
        itemData.icon = loadedData.icon
        changed = true
    end

    if nameText then
        nameText:SetText(itemData.name or ("Item " .. itemID))
    end
    if iconTexture then
        iconTexture:SetTexture(itemData.icon or "Interface\\Icons\\INV_Misc_QuestionMark")
    end

    if changed and ns.Items and ns.Items.SaveItem then
        ns.Items:SaveItem(itemID, itemData)
    end

    return loadedData.name ~= nil or loadedData.icon ~= nil
end

local function ApplyItemDisplayData(itemID, itemData, nameText, iconTexture)
    itemID = tonumber(itemID)
    ...
    if cached and cached.name then
        return ApplyLoadedItemData(itemID, itemData, cached, nameText, iconTexture)
    end

    local itemName, itemLink, itemRarity, itemLevel, _, itemType, itemSubType, _, _, itemTexture
    if C_Item and C_Item.GetItemInfo then
        itemName, itemLink, itemRarity, itemLevel, _, itemType, itemSubType, _, _, itemTexture = C_Item.GetItemInfo(itemID)
    else
        itemName, itemLink, itemRarity, itemLevel, _, itemType, itemSubType, _, _, itemTexture = GetItemInfo(itemID)
    end

    local changed = false
    if itemName and itemName ~= "" and IsPlaceholderItemName(itemData.name, itemID) then
        itemData.name = itemName
        changed = true
    end
    if itemLink and itemData.link ~= itemLink then
        itemData.link = itemLink
        changed = true
    end
    if itemRarity and itemData.rarity ~= itemRarity then
        itemData.rarity = itemRarity
        itemData.quality = itemRarity
        changed = true
    end
    if itemLevel and itemData.level ~= itemLevel then
        itemData.level = itemLevel
        changed = true
    end
    if itemType and itemData.type ~= itemType then
        itemData.type = itemType
        changed = true
    end
    if itemSubType and itemData.subType ~= itemSubType then
        itemData.subType = itemSubType
        changed = true
    end
    if itemTexture and itemData.icon ~= itemTexture then
    ...
    return itemName ~= nil or itemTexture ~= nil
end

local function RequestItemDisplayData(itemID, itemData, nameText, iconTexture)
    itemID = tonumber(itemID)
    if not itemID or not itemData then return end

    if ApplyItemDisplayData(itemID, itemData, nameText, iconTexture) then
        itemDisplayAttempts[itemID] = nil
        return
    end

    local loader = GetItemDataLoader()
    if loader and loader.LoadItemData then
        loader:LoadItemData(itemID, function(_, loadedData)
            itemDisplayAttempts[itemID] = nil
            local freshData = ns.Items and ns.Items:GetItem(itemID) or itemData
            ApplyLoadedItemData(itemID, freshData, loadedData, nameText, iconTexture)
        end)
        return
    end
    ...
    local attempts = (itemDisplayAttempts[itemID] or 0) + 1
    itemDisplayAttempts[itemID] = attempts
    if attempts > 6 or not C_Timer or not C_Timer.NewTimer then
        return
    end

    C_Timer.NewTimer(0.25 * attempts, function()
        if ns.Items then
            local freshData = ns.Items:GetItem(itemID) or itemData
            RequestItemDisplayData(itemID, freshData, nameText, iconTexture)
        end
    end)
end
...
function ns.UI.CreateItemsTab(parent)
    ...
    local function ShowEditor()
        ...
        if selectedItem and ns.Items then
            ...
            if itemData then
                ...
                RequestItemDisplayData(selectedItem, itemData, header.nameText, header.itemIcon)
                ...
                if header.itemIconFrame then
                    ...
                    header.itemIconFrame:SetScript("OnMouseDown", function()
                        HandleItemPreviewClick(selectedItem, itemData.link)
                    end)
                end
    ...
    function parent.RefreshItemsList()
        ...
        local function BuildItemRow(item, yOffset)
            ...
            RequestItemDisplayData(item.id, item.data, titleText, icon)
            ...
            row:SetScript("OnMouseDown", function()
                if HandleItemPreviewClick(item.id, item.data.link) then
                    return
                end
    ...
    ns.UI.OpenNotesItem = function(itemID)
        itemID = tonumber(itemID)
        if not itemID or not ns.Items or not ns.Items:GetItem(itemID) then
            return false
        end
