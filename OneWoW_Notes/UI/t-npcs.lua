...
local npcQuestCache = nil

local function GetNPCZoneDisplay(npcData)
    if not npcData then return "" end
    if npcData.mapID and C_Map and C_Map.GetMapInfo then
        local mapInfo = C_Map.GetMapInfo(npcData.mapID)
        if mapInfo and mapInfo.name and mapInfo.name ~= "" then
            return mapInfo.name
        end
    end
    return npcData.zone or ""
end

local function GetQuestDataAddon()
    local catalog = _G.OneWoW_Catalog
    if catalog and catalog.Catalog and catalog.Catalog.GetDataAddon then
        return catalog.Catalog:GetDataAddon("quests")
    end
    return nil
end

local function AddQuestForNPC(cache, npcID, questID, quest)
    npcID = tonumber(npcID)
    questID = tonumber(questID)
    if not npcID or not questID then return end

    cache[npcID] = cache[npcID] or {}
    cache[npcID][questID] = quest
end

local function BuildNPCQuestCache()
    local cache = {}
    local questAddon = GetQuestDataAddon()

    if not (questAddon and questAddon.QuestData and questAddon.QuestData.GetAllQuests) then
        return cache
    end

    for questID, quest in pairs(questAddon.QuestData:GetAllQuests()) do
        if quest.starts then
            for _, starter in ipairs(quest.starts) do
                AddQuestForNPC(cache, starter.npcID, questID, quest)
            end
        end

        if quest.ends then
            for _, ender in ipairs(quest.ends) do
                AddQuestForNPC(cache, ender.npcID, questID, quest)
            end
        end

        if quest.questGiverID then
            AddQuestForNPC(cache, quest.questGiverID, questID, quest)
        end
    end
...
local function GetNPCQuests(npcID)
    npcID = tonumber(npcID)
    if not npcID then return {} end

    local cache = npcQuestCache or BuildNPCQuestCache()
    local questLookup = cache[npcID] or {}
    local quests = {}

    for questID, quest in pairs(questLookup) do
        table.insert(quests, {
            id = questID,
            name = quest and quest.name or ("Quest " .. tostring(questID)),
        })
    end
...
local function OpenCatalogQuest(questID)
    questID = tonumber(questID)
    if not questID then return end

    if _G.OneWoW_Catalog
        and OneWoW_Catalog.UI
        and OneWoW_Catalog.UI.OpenQuest
    then
        OneWoW_Catalog.UI.OpenQuest(questID)
    elseif _G.OneWoW and OneWoW.GUI and OneWoW.GUI.Show then
        OneWoW.GUI:Show("catalog")
        if OneWoW.GUI.SelectSubTab then
            OneWoW.GUI:SelectSubTab("catalog", "quests")
        end
        print("|cFFFFD100OneWoW - NPCs:|r Catalog quest UI is still loading.")
    end
end
...
function ns.UI.CreateNPCsTab(parent)
    ...
    local npcNameUIRefreshPending = false
    local pendingNPCNameIDs = {}

    local function RegisterVisibleNPCName(npcID, nameText)
        npcID = tonumber(npcID)
        if not npcID or not nameText then
            return
        end

        visibleNPCNameRows[npcID] = visibleNPCNameRows[npcID] or {}
        table.insert(visibleNPCNameRows[npcID], nameText)
    end

    local function ApplyVisibleNPCName(npcID)
        npcID = tonumber(npcID)
        ...
        if rows then
            local remaining = {}
            for _, nameText in ipairs(rows) do
                if nameText and nameText.SetText then
                    nameText:SetText(npcName)
                    applied = true
                    table.insert(remaining, nameText)
                end
            end
            if #remaining > 0 then
        ...
        if selectedNPC == npcID
            and detailPanel
            and detailPanel.editorContent
            and detailPanel.editorContent.header
            and detailPanel.editorContent.header.nameText
        then
            detailPanel.editorContent.header.nameText:SetText(npcName)
            applied = true
        end
    ...
    addTargetBtn:SetScript("OnClick", function()
        if ns.NPCs then
            ...
            if ns.NPCs:GetNPC(npcInfo.id) then
                ns.NPCs:EnsureNPC(npcInfo.id, npcInfo)
    ...
    local function PopulateNPCQuestLinks(section, npcID)
        if not section then return end

        section.questRows = section.questRows or {}
        for _, row in ipairs(section.questRows) do
            row:Hide()
        end
        section.questRows = {}

        local scrollChild = section.questScrollChild or section
        local quests = GetNPCQuests(npcID)
        if #quests == 0 then
            section:Hide()
            return
        end
        ...
        section:Show()
        if section.questScroll then section.questScroll:Show() end
        ...
        for _, quest in ipairs(quests) do
            ...
            text:SetText(string.format("%d - %s", quest.id, quest.name or ("Quest " .. tostring(quest.id))))
            text:SetTextColor(OneWoW_GUI:GetThemeColor("TEXT_ACCENT"))

            row:SetScript("OnClick", function()
                OpenCatalogQuest(quest.id)
            end)
            row:SetScript("OnEnter", function(self)
                text:SetTextColor(1, 0.82, 0)
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                GameTooltip:AddLine(quest.name or ("Quest " .. tostring(quest.id)), 1, 0.82, 0)
                GameTooltip:AddLine("Quest ID: " .. tostring(quest.id), 0.6, 0.6, 0.6)
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine("Click to open this quest in Catalog", 0, 1, 0)
                GameTooltip:Show()
            end)
            ...
            table.insert(section.questRows, row)
            y = y - 22
        end

        if section.questScrollChild then
            section.questScrollChild:SetHeight(math.max(40, (#quests * 22) + 8))
        end

        if section.questScroll and section.questScroll.UpdateScrollChildRect then
            section.questScroll:UpdateScrollChildRect()
        end
    end
    ...
    local function ShowEditor()
        ...
        if not detailPanel.editorContent then
            ...
            local locationBtn = CreateFrame("Button", nil, editorHeader)
            locationBtn:SetPoint("TOPLEFT", idText, "BOTTOMLEFT", 0, -2)
            locationBtn:SetSize(320, 14)

            local locationText = OneWoW_GUI:CreateFS(locationBtn, 10)
            locationText:SetAllPoints()
            ...
            locationBtn:SetScript("OnEnter", function(self)
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                GameTooltip:SetText(L["UI_NPC_GOTO_TITLE"] or "Open Map", 1, 1, 1)
                GameTooltip:AddLine(L["UI_NPC_CREATE_WAYPOINT"] or "Open the map and set a waypoint for this NPC.", 0.8, 0.8, 0.8, true)
                GameTooltip:Show()
            end)
            locationBtn:SetScript("OnLeave", function() GameTooltip:Hide() end)
            editorHeader.locationText = locationText
            editorHeader.locationBtn = locationBtn
            ...
            local questSection = CreateThemedBar(nil, detailPanel)
            questSection:SetPoint("TOPLEFT", tooltipSection, "BOTTOMLEFT", 0, -10)
            questSection:SetPoint("TOPRIGHT", tooltipSection, "BOTTOMRIGHT", 0, -10)
            questSection:SetPoint("BOTTOMLEFT", detailPanel, "BOTTOMLEFT", 10, 10)
            questSection:SetPoint("BOTTOMRIGHT", detailPanel, "BOTTOMRIGHT", -10, 10)

            local questLabel = OneWoW_GUI:CreateFS(questSection, 12)
            questLabel:SetPoint("TOPLEFT", questSection, "TOPLEFT", 10, -8)
            questLabel:SetText("Associated Quests:")
            questLabel:SetTextColor(OneWoW_GUI:GetThemeColor("ACCENT_PRIMARY"))
            questSection.questLabel = questLabel

            local questScroll = OneWoW_GUI:CreateScrollFrame(questSection, {})
            questScroll:SetPoint("TOPLEFT", questSection, "TOPLEFT", 10, -30)
            questScroll:SetPoint("BOTTOMRIGHT", questSection, "BOTTOMRIGHT", -26, 8)

            local questScrollChild = CreateFrame("Frame", nil, questScroll)
            questScrollChild:SetPoint("TOPLEFT", questScroll, "TOPLEFT", 0, 0)
            questScrollChild:SetPoint("TOPRIGHT", questScroll, "TOPRIGHT", 0, 0)
            questScrollChild:SetHeight(40)
            questScroll:SetScrollChild(questScrollChild)

            questScroll:SetScript("OnSizeChanged", function(self, width)
                questScrollChild:SetWidth(width or self:GetWidth())
                if self.UpdateScrollChildRect then
                    self:UpdateScrollChildRect()
                end
            end)

            questSection.questScroll = questScroll
            questSection.questScrollChild = questScrollChild
        ...
        if selectedNPC and ns.NPCs then
            ...
            if nd then
                ...
                if header.idText then
                    ...
                    local zoneName = GetNPCZoneDisplay(nd)
                    if zoneName ~= "" then idStr = idStr .. "  Zone: " .. zoneName end
                ...
                PopulateNPCQuestLinks(detailPanel.editorContent.questSection, selectedNPC)
            end
        end
    end
    ...
    local function QueueNPCNameUIRefresh(resolvedNPCID)
        resolvedNPCID = tonumber(resolvedNPCID)
        if resolvedNPCID then
            pendingNPCNameIDs[resolvedNPCID] = true
        end

        if npcNameUIRefreshPending then return end
        npcNameUIRefreshPending = true

        local function refresh()
            npcNameUIRefreshPending = false
            if not parent or (parent.IsShown and not parent:IsShown()) then
                return
            end

            for npcID in pairs(pendingNPCNameIDs) do
                ApplyVisibleNPCName(npcID)
                pendingNPCNameIDs[npcID] = nil
            end
        end
    ...
    function parent.RefreshNPCsList()
        ...
        if ns.NPCs.ResolveStoredNPCName and selectedNPC then
            ns.NPCs:ResolveStoredNPCName(selectedNPC, QueueNPCNameUIRefresh)
        end
        if ns.NPCs.RefreshMissingNames then
            ns.NPCs:RefreshMissingNames(QueueNPCNameUIRefresh, 20)
        end
        ...
        local function sortNPCs(a, b)
            ...
            if currentSort.by == "zone" then
                local za = GetNPCZoneDisplay(a.data)
                local zb = GetNPCZoneDisplay(b.data)
        ...
        local NPC_ROW_HEIGHT = 68
        local NPC_ROW_STEP = 73

        local function BuildNPCRow(npc, yOffset, groupArray, groupIndex)
            local row = CreateFrame("Frame", nil, scrollChild, "BackdropTemplate")
            row:SetSize(scrollChild:GetWidth(), NPC_ROW_HEIGHT)
            ...
            RegisterVisibleNPCName(npc.id, nameText)

            local subText = OneWoW_GUI:CreateFS(row, 10)
            subText:SetPoint("TOPLEFT", row, "TOPLEFT", 10, -28)
            subText:SetPoint("TOPRIGHT", row, "TOPRIGHT", -10, -28)
            subText:SetJustifyH("LEFT")
            subText:SetWordWrap(false)
            subText:SetText(GetNPCZoneDisplay(npc.data))
        ...
        for i, n in ipairs(newNPCs) do BuildNPCRow(n, yOffset, newNPCs, i) yOffset = yOffset - NPC_ROW_STEP end
        ...
        for i, n in ipairs(favorites) do BuildNPCRow(n, yOffset, favorites, i) yOffset = yOffset - NPC_ROW_STEP end
        ...
        for i, n in ipairs(regular) do BuildNPCRow(n, yOffset, regular, i) yOffset = yOffset - NPC_ROW_STEP end
...
function ns.UI.ShowNPCPropertiesDialog(npcID, refreshParent)
    ...
    zoneDisplay:SetText(GetNPCZoneDisplay(nd) ~= "" and GetNPCZoneDisplay(nd) or "?")
