...
local function GetScanTooltipItemName(itemID)
    ...
    local titleLine = _G.OneWoWQuestRewardItemScanTooltipTextLeft1
    local text =
        titleLine
...
function ShowQuestDetail(panels, questData)
    ...
    local function addNPCNavigationRow(label, npcData)
        ...
        local npcName =
            ...
            or ("NPC #" .. tostring(npcData.npcID))

        if npcID and npcName == ("NPC #" .. tostring(npcData.npcID)) then
            ScheduleNPCNameRefresh(npcID, questData)
        end
    ...
    questIDText:SetText(string.format("%s: %d  |  ", L["QUESTS_QUESTID"], questID))
                ...
                if currencyID then
                    ...
                    currencyFrame:SetScript("OnEnter", function(self)
                        ...
                        if not shownTooltip then
                            GameTooltip:AddLine(currencyName, 1, 0.82, 0)
                        end
        ...
        RequestVisibleRewardItem(itemID, questData)
                    ...
                    itemFrame:SetScript("OnEnter", function(self)
                        ...
                        if itemName == ("Item #" .. tostring(itemID)) then
                            GameTooltip:AddLine(
                                string.format(
                                    "OneWoW resolving: %d/%d%s%s",
                                    rewardItemPollAttempts[itemID] or 0,
                                    REWARD_ITEM_POLL_ATTEMPTS,
                                    pendingRewardItemIDs[itemID] and " pending" or "",
                                    queuedRewardItemIDs[itemID] and " queued" or ""
                                ),
                                0.6,
                                0.8,
                                1
                        ...
                        GameTooltip:AddLine("Click to open in Item Search", 0, 1, 0)
