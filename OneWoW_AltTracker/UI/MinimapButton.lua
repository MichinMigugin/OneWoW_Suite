...
local ICON_HORDE    = "Interface\\AddOns\\OneWoW_AltTracker\\Media\\horde-mini.png"
local ICON_ALLIANCE = "Interface\\AddOns\\OneWoW_AltTracker\\Media\\alliance-mini.png"
local ICON_NEUTRAL  = "Interface\\AddOns\\OneWoW_AltTracker\\Media\\neutral-mini.png"
...
function MinimapButton:Initialize()
    ...
    local plugin = ldb:NewDataObject("OneWoW_AltTracker", {
        type = "launcher",
        icon = GetCurrentIcon(),
        ...
        OnEnter = function(self)
            ...
            GameTooltip:AddLine("|cFFFFD100OneWoW|r - AltTracker", 1, 0.82, 0, 1)
    ...
    if libDBIcon and addon and addon.db and addon.db.global and addon.db.global.minimap then
        libDBIcon:Register("OneWoW_AltTracker", plugin, addon.db.global.minimap)
    end
end

function MinimapButton:Show()
    if libDBIcon then
        libDBIcon:Show("OneWoW_AltTracker")
    end
end

function MinimapButton:Hide()
    if libDBIcon then
        libDBIcon:Hide("OneWoW_AltTracker")
    end
end
...
function MinimapButton:IsShown()
    if libDBIcon then
        local btn = libDBIcon:GetMinimapButton("OneWoW_AltTracker")
