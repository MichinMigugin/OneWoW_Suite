...
function CursorEnhancerModule:CreateCustomDetail(detailScrollChild, yOffset, isEnabled, registerRefresh)
    ...
    local root = NewStack(detailScrollChild, yOffset, 12)

    local function SyncRootY()
        yOffset = root:GetY()
    end

    local function Section(titleText)
        SyncRootY()
        yOffset = yOffset - SECTION_TOP_PAD
        yOffset = OneWoW_GUI:CreateSection(detailScrollChild, {
            title = titleText, yOffset = yOffset,
        })
        root = NewStack(detailScrollChild, yOffset, 12)
    end

    --- Soft inset card for a look group (Outer / Middle / Marker / Trail).
    local function BeginGroup(titleText)
        SyncRootY()
        yOffset = yOffset - 8
        local card = OneWoW_GUI:CreateFrame(detailScrollChild, {
            backdrop = OneWoW_GUI.Constants.BACKDROP_INNER_NO_INSETS,
            bgColor = "BG_TERTIARY",
            borderColor = "BORDER_SUBTLE",
            height = 40,
        })
        card:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 8, yOffset)
        card:SetPoint("TOPRIGHT", detailScrollChild, "TOPRIGHT", -8, yOffset)
    ...
    -- ─── Global look / feel ──────────────────────────────────────────────────
    Section(L["CURSORENHANCER_SECTION_GLOBAL"])
    ...
    -- Outer Ring
    do
        local g, finish = BeginGroup(L["CURSORENHANCER_THING_OUTER"])
    ...
    -- Middle Ring
    do
        local g, finish = BeginGroup(L["CURSORENHANCER_THING_MIDDLE"])
    ...
    -- Center Marker
    do
        local g, finish = BeginGroup(L["CURSORENHANCER_THING_MARKER"])
        local colorLbl, colorSwatch
    ...
    -- Trail
    do
        local g, finish = BeginGroup(L["CURSORENHANCER_THING_TRAIL"])
    ...
    SyncRootY()
    Section(L["CURSORENHANCER_SECTION_SWIPES"])
    local middleFillCb
    ...
    SyncRootY()
    Section(L["CURSORENHANCER_SECTION_PIPS"])
    ...
    local function SwipeDefaults(sectionKey, cfg, enableLabel, apply, hasSpark)
        SyncRootY()
        ...
        -- Class color sits under Color (override child)
        RefreshDeps()
    end

    SwipeDefaults("CURSORENHANCER_SECTION_GCD", s.gcd, L["CURSORENHANCER_ENABLE_GCD"],
        function() CursorEnhancerModule.CE:ApplyGCD() end, false)
    SwipeDefaults("CURSORENHANCER_SECTION_CAST", s.castCircle, L["CURSORENHANCER_ENABLE_CAST"],
        function() CursorEnhancerModule.CE:ApplyCast() end, true)

    -- ─── Situations ──────────────────────────────────────────────────────────
    Section(L["CURSORENHANCER_SECTION_SITUATIONS"])
    SyncRootY()

    local conflictAlert = detailScrollChild:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    conflictAlert:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 12, yOffset)
    conflictAlert:SetPoint("TOPRIGHT", detailScrollChild, "TOPRIGHT", -12, yOffset)
    ...
    conflictAlert:Hide()
    local conflictAlertY = yOffset
    yOffset = yOffset - 8

    local addBtn = OneWoW_GUI:CreateFitTextButton(detailScrollChild, {
        text = L["CURSORENHANCER_ADD_SITUATION"],
    })
    addBtn:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 12, yOffset)
    ...
    situationsHost = CreateFrame("Frame", nil, detailScrollChild, "BackdropTemplate")
    situationsHost:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 8, yOffset)
    situationsHost:SetPoint("TOPRIGHT", detailScrollChild, "TOPRIGHT", -8, yOffset)
    ...
    rebuildSituations = function()
        ...
        if hasConflict then
            ...
            conflictAlert:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 12, conflictAlertY)
            conflictAlert:SetPoint("TOPRIGHT", detailScrollChild, "TOPRIGHT", -12, conflictAlertY)
        else
            conflictAlert:Hide()
        end
        ...
        -- Stretch scroll child so new cards are reachable.
        local hostBottom = situationsHost:GetBottom()
        local childBottom = detailScrollChild:GetBottom()
        if hostBottom and childBottom then
            local need = (detailScrollChild:GetTop() - hostBottom) + 24
            if need > detailScrollChild:GetHeight() then
                detailScrollChild:SetHeight(need)
            end
        end
    end
    ...
    rebuildSituations()
    yOffset = yOffset - situationsHost:GetHeight() - 12
