local _, ns = ...
if not ns.L_enUS then ns.L_enUS = {} end
local L = ns.L_enUS

L["BAGBAR_TITLE"]                         = "Bag Bar"
L["BAGBAR_DESC"]                          = "Shows usable bag items on a movable bar. Items are chosen with a keyword expression (same as Bag search). Equippable gear and quest items are always excluded from the bar (applied automatically, not shown in the editor)."
L["BAGBAR_DRAG_TOOLTIP"]                  = "Click and drag to move"
L["BAGBAR_LOCK_POSITION"]                 = "Lock Position"
L["BAGBAR_MAX_BUTTONS"]                   = "Maximum Buttons"
L["BAGBAR_BUTTON_SIZE"]                   = "Button Size"
L["BAGBAR_LEFT_CLICK_TO_USE"]             = "Left-click to use"
L["BAGBAR_SHIFT_RIGHT_CLICK_TO_SKIP"]     = "Shift+Right-click to skip this session"
L["BAGBAR_ALT_RIGHT_CLICK_TO_BLACKLIST"]  = "Alt+Right-click to permanently blacklist"
L["BAGBAR_SETTINGS_HEADER"]               = "Bar Settings"
L["BAGBAR_MANUAL_ITEMS_HEADER"]           = "Manual Items"
L["BAGBAR_MANUAL_DESC"]                   = "Pin specific items for higher priority in the bar. They still must match your expression filter and bar usability rules."
L["BAGBAR_MACROS_HEADER"]                 = "Manual Macros"
L["BAGBAR_MACROS_DESC"]                   = "Add your macros to the bar as custom buttons. Drag a macro from the macro window onto the drop area, or type a macro name and click Add. Macros appear before bag items."
L["BAGBAR_MACRO_NAME_LABEL"]              = "Macro Name:"
L["BAGBAR_DRAG_MACRO_HERE"]               = "Drag Macro Here"
L["BAGBAR_MACRO_LEFT_CLICK_TO_RUN"]       = "Left-click to run macro"
L["BAGBAR_MACRO_MISSING"]                 = "(missing)"
L["BAGBAR_ITEM_ID_LABEL"]                 = "Item ID:"
L["BAGBAR_ADD_BUTTON"]                    = "Add"
L["BAGBAR_DRAG_ITEM_HERE"]                = "Drag Item Here"
L["BAGBAR_BLACKLIST_HEADER"]              = "Blacklist"
L["BAGBAR_BLACKLIST_DESC"]                = "Shift+Right-click items in the bar to skip for this session. Alt+Right-click to permanently blacklist."
L["BAGBAR_CLEAR_BLACKLIST"]               = "Clear Blacklist"
L["BAGBAR_BLACKLIST_CLEARED"]             = "Bag Bar blacklist cleared."
L["BAGBAR_SHOW_BAR"]                      = "Show Bar"
L["BAGBAR_HIDE_BAR"]                      = "Hide Bar"
L["BAGBAR_CLEAR_BLACKLIST_CONFIRM"]       = "Are you sure you want to clear the entire Bag Bar blacklist?"
L["BAGBAR_ICON_SPACING"]                 = "Icon Spacing"
L["BAGBAR_COLUMNS"]                      = "Columns"
L["BAGBAR_CONTEXT_LOCK"]                 = "Lock Position"
L["BAGBAR_CONTEXT_UNLOCK"]               = "Unlock Position"
L["BAGBAR_CONTEXT_SETTINGS"]             = "Open Settings"
L["BAGBAR_HIDE_ANCHOR"]                  = "Hide Anchor (show on hover)"
L["BAGBAR_GROW_DIRECTION"]               = "Grow Direction"
L["BAGBAR_GROW_RIGHT"]                   = "Right"
L["BAGBAR_GROW_LEFT"]                    = "Left"
L["BAGBAR_GROW_DOWN"]                    = "Down"
L["BAGBAR_GROW_UP"]                      = "Up"
L["BAGBAR_EXPRESSION_FILTER_HEADER"]      = "Expression Filter"
L["BAGBAR_EXPRESSION_FILTER_DESC"]        = "Keyword expression for which bag items appear (same keywords as Bag search). Click ? for help. Equippable gear and quest items are excluded automatically from this expression."
L["BAGBAR_EXPRESSION_FILTER_PLACEHOLDER"] = "e.g. #usable & #mount"
