...
- **Restriction gating:** every `UseContainerItem` move path (`DepositBagButtonStack`, `TransferSearchToBank` / `FromBank`, and the paced deposit/withdraw tickers) gates on `OneWoW.Restriction.IsProtectedActionBlocked()` — entry points bail when blocked, and each ticker re-checks per tick so a transfer that starts safely pauses (and resumes) if combat begins mid-queue. `IsProtectedActionBlocked` excludes the `Map` restriction, so bank transfers keep working inside a Delve out of combat. Money transfers (`C_Bank.WithdrawMoney`/`DepositMoney`) are not combat-protected and are left ungated, matching Blizzard's own `BankFrame` popups.
