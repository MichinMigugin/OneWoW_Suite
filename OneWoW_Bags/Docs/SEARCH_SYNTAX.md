...
| `#combinable` | Item whose Use: effect is a combine/craft spell with required reagents (Darkmoon decks, fractured sparks, torn recipe scraps, …). Structural via `GetItemSpell` → recipe schematic — not tooltip text. Enchant scrolls (`ItemEnhancement`) are excluded. Does **not** check whether you own the reagents |
