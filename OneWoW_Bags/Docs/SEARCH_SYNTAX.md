...
| `#combinable` | Item whose Use: effect is a combine/craft spell with required reagents (Darkmoon decks, fractured sparks, torn recipe scraps, …). Structural via `GetItemSpell` → recipe schematic — not tooltip text. Enchant scrolls (`ItemEnhancement`) are excluded. Compose with `#usable` for ready-to-combine (`#combinable & #usable`) |
...
> **`#usable` vs `#onuse` vs `#combinable`:** `#usable` / `#unusable` are character-eligibility (`IsUsableItem` in bags; bank fallback: combine items match when all reagents are owned, teachable collectibles while still learnable, others need a direct `Use:` tooltip — not recipes/non-collectible teach items). `#onuse` means the tooltip has a **Use:** effect — independent of whether you meet the requirements. `#combinable` means the item is a combine-type item (has a reagent schematic); it does **not** check whether you own the reagents — use `#combinable & #usable` for ready-to-combine. `#use` is not a registered keyword (use `#usable` or `#onuse` explicitly).
