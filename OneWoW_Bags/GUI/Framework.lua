...
function GUI:CreateButton(name, parent, text, width, height)
    local C = OneWoW_Bags.Constants and OneWoW_Bags.Constants.GUI or { BUTTON_HEIGHT = 28 }
    ...
    btn.AutoFit = function(self, padding)
        padding = padding or 16
        local textWidth = self.text:GetStringWidth()
        self:SetWidth(textWidth + padding)
    end
...
function GUI:CreateEditBox(name, parent, width, height)
    local C = OneWoW_Bags.Constants and OneWoW_Bags.Constants.GUI or { SEARCH_HEIGHT = 32 }
...
GUI.GetThemeColor = T
GUI.GetSpacing = S
GUI.BACKDROP_SOFT = BACKDROP_SOFT
GUI.BACKDROP_INNER = BACKDROP_INNER
